    ...
    return (
        <div className="p-6 max-w-6xl mx-auto space-y-8 animate-fade-in bg-[#fdfdfd] min-h-screen text-gray-800 font-sans">
            ...
            <div className="flex justify-between items-center border-b border-gray-100 pb-6">
                <div>
                    <h1 className="text-xl font-medium tracking-tight text-gray-900">
                        {t('admin.system_overview').split(' ')[0]} <span className="text-[#009A44] font-bold">{t('admin.system_overview').split(' ').slice(1).join(' ')}</span>
                    </h1>
                ...
                <div className="flex items-center gap-4">
                    <div className="text-[10px] font-semibold text-gray-400 bg-white px-3 py-1.5 rounded-full border border-gray-100 flex items-center gap-2">
                        <Clock size={12} className="text-[#009A44]" /> {t('admin.sync')}: {stats.lastUpdate}
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    { label: t('nav.players'), value: stats.totalPlayers, icon: <Users size={18} />, theme: "border-green-100 hover:bg-green-50/30", color: "text-[#009A44]" },
                    { label: t('nav.events'), value: stats.totalEvents, icon: <Calendar size={18} />, theme: "border-yellow-100 hover:bg-yellow-50/30", color: "text-yellow-600" },
                    { label: t('nav.insights'), value: stats.totalInsights, icon: <Newspaper size={18} />, theme: "border-red-100 hover:bg-red-50/30", color: "text-[#FF0000]" },
                ].map((item, i) => (
                    <div key={i} className={`p-6 bg-white border ${item.theme} rounded-2xl transition-all duration-300 group cursor-default`}>
                        ...
                        <div className="flex items-baseline gap-2">
                            ...
                            <span className="text-[10px] text-gray-400 font-medium">{t('admin.total_entries')}</span>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className="lg:col-span-2 bg-white border border-gray-100 rounded-3xl p-8">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-sm font-bold text-gray-900 uppercase tracking-wider">{t('admin.metrics_health')}</h3>
                        <ExternalLink size={14} className="text-gray-300 hover:text-[#009A44] cursor-pointer" />
                    </div>
                    <div className="space-y-6">
                        {[
                            { label: t('admin.profile_integrity'), progress: 85, color: "bg-[#009A44]" },
                            { label: t('admin.calendar_accuracy'), progress: 62, color: "bg-[#FEDD00]" },
                            { label: t('admin.media_optimization'), progress: 40, color: "bg-[#FF0000]" },
                        ].map((goal, i) => (
                            <div key={i}>
                ...
                <div className="bg-gray-900 rounded-3xl p-8 text-white relative overflow-hidden flex flex-col justify-between">
                    ...
                    <div className="relative z-10">
                        <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">{t('admin.express_actions')}</h3>
                        <div className="space-y-3">
                            <button className="w-full flex items-center justify-between p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl transition-all group">
                                <span className="text-[11px] font-bold uppercase tracking-wide">{t('admin.add_player')}</span>
                                <ArrowRight size={14} className="text-[#009A44] group-hover:translate-x-1 transition-transform" />
                            </button>
                            <button className="w-full flex items-center justify-between p-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl transition-all group">
                                <span className="text-[11px] font-bold uppercase tracking-wide">{t('admin.sync_calendar')}</span>
                    ...
                    <div className="relative mt-8 pt-6 border-t border-white/5 flex items-center justify-between">
                        <span className="text-[9px] font-medium text-gray-500 tracking-wider">{t('admin.secure_instance')} v2.4</span>
